-- Load data from HDFS
inputDialogues4 = LOAD 'hdfs:///user/puja/inputs/episodeIV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues5 = LOAD 'hdfs:///user/puja/inputs/episodeV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues6 = LOAD 'hdfs:///user/puja/inputs/episodeVI_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
...
rmf hdfs:///user/puja/outputs;

-- Store result in HDFS
STORE namesOrdered INTO 'hdfs:///user/puja/outputs' USING PigStorage('\t');